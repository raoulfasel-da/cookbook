...
# Save the model
saveRDS(xgb_fit, "r-xgboost-recipe/xgb_fit.RDS")
...
# Zip the file
zip(zipfile = "r-xgboost-recipe", files = "r-xgboost-recipe") # Note: this function does not work with Windows 10, please zip the file manually if you are working with that OS.
r_xgboost_recipe_zip <- file.path(getwd(), "r-xgboost-recipe.zip")  #Make sure the working directory is set to the directory which contains the deployment_package.zip
